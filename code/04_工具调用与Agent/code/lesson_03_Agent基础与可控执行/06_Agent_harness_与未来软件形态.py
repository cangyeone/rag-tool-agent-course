...
class AgentHarness:
    ...
    def _maybe_call_tool(self, user_input: str) -> dict:
        ...
        if "票" in user_input or "G" in user_input:
            return {"tool": "query_tickets", "result": {"G107": "售罄", "G109": "有票"}}
        if "站" in user_input or "编码" in user_input:
            return {"tool": "station_code", "result": {"北京南": "VNP"}}
        return {}
    ...
    def _generate_answer(self, user_input: str, tool_result: dict) -> str:
        ...
        if tool_name == "query_tickets":
            return (f"查询结果：G107 售罄，G109 有票。"
                    f"最终以 官方页面为准。")
        if tool_name == "station_code":
...
print("【单 Agent 运行】")
r1 = harness.run(sid, "查 G107 明天有票吗")
print(f"  Q: 查 G107 明天有票吗")
print(f"  A: {r1['answer']}")

r2 = harness.run(sid, "北京南的编码")
print(f"  Q: 北京南的编码")
print(f"  A: {r2['answer']}")
...
class MultiAgentSystem:
    ...
    def __init__(self):
        self.ticket_agent = AgentHarness("ticket_agent", {"query": "查票", "order": "下单"})
    ...
    def dispatch(self, user_input: str, user_id: str) -> str:
        ...
        elif any(w in user_input for w in ["票", "订单编号", "G", "D"]):
            return self.ticket_agent.run(
                self.ticket_agent.create_session(user_id), user_input)["answer"]
        else:
            return "请描述与 示例业务系统 相关的问题。"

system = MultiAgentSystem()
print(f"  下单请求: {system.dispatch('帮我查 G107 明天有票吗', 'user_001')}")
...
future_trends = [
    ...
    ("从人工操作到自主执行", "不只是'回答问题'，而是'帮你把事办了'（下单、退款、变更）"),
