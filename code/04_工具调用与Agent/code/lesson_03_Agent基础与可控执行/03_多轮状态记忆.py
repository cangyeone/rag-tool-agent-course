...
# 第一轮：用户查票
print("【第 1 轮：用户查票】")
q1 = "帮我查明天 G107 从北京南出发的票"
session.add_message("user", q1)
# 模拟 Agent 调用工具
session.record_tool_call("query_tickets",
                         {"train_no": "G107", "date": "2026-06-20"},
                         {"remaining": 0, "status": "售罄"})
session.set_context("last_query_train", "G107")
session.set_context("last_query_date", "2026-06-20")
session.set_context("last_query_result", "售罄")
session.add_message("assistant", "G107 明日已售罄，需要帮您查看其他订单编号吗？")
print(f"  用户: {q1}")
print(f"  [工具调用] query_tickets(G107) → 售罄")
print(f"  状态: {json.dumps(session.summarize(), ensure_ascii=False)}")

# 第二轮：用户追问（Agent 必须"记得"上一轮在聊 G107）
print("\n【第 2 轮：用户追问（依赖状态）】")
q2 = "那帮我候补申请吧"
session.add_message("user", q2)
# Agent 从状态中恢复上下文
train_no = session.get_context("last_query_train")  # "G107"
date = session.get_context("last_query_date")        # "2026-06-20"
print(f"  用户: {q2}")
print(f"  [状态恢复] 上一轮查询的是 {train_no}，日期 {date}")
# Agent 现在知道候补申请的是 G107，而不是问"哪趟车"
...
session.add_message("assistant",
                    f"已为您提交 {train_no} 候补申请订单，当前排队第 5 位。")
print(f"  [工具调用] submit_waitlist({train_no}) → 排队第 5 位")
print(f"  Agent: 已为您提交 {train_no} 的候补申请，排队第 5 位")
...
if has_waitlist:
    ...
    session.add_message("assistant",
                        f"您候补申请的是 {train_no}，当前排队第 {last_call['result']['position']} 位。")
...
print("\n状态记忆设计要点：")
print("1. context 存关键信息（上次查询的订单编号、日期、结果），避免重复调用工具")
