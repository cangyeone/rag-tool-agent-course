...
def query_tickets(args: dict) -> dict:
    ...
    return {"trains": [{"no": "G107", "left": 5}, {"no": "G109", "left": 0}], "source": "示例业务系统实时查询"}
...
def calc_refund(args: dict) -> dict:
    ...
    return {"fee": round(args["ticket_price"] * rate, 2), "rate": rate, "based_on": "中国行业退款规定"}
...
result = query_tickets({"from_station": "北京南", "to_station": "上海虹桥"})
tool_msg = {
    "role": "tool",
...
result = empty_query({})
if isinstance(result, dict) and result.get("trains") == []:
    print(f"  空结果处理：告知模型该区间当前无库存，建议扩大日期范围或换订单编号")

# ========== 场景4：多工具并行时汇总结果 ==========
print("\n【场景4：多工具并行 → 汇总所有结果再回传】")
parallel_calls = [
    {"id": "p1", "tool": "query_tickets", "args": {"from_station": "北京南", "to_station": "上海虹桥"}},
    {"id": "p2", "tool": "calc_refund", "args": {"ticket_price": 553, "hours_before_departure": 24}},
]
responses = []
tool_map = {"query_tickets": query_tickets, "calc_refund": calc_refund}
