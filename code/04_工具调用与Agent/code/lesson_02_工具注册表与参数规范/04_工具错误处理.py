...
def mock_tool(tool_name, args):
    ...
    if tool_name == "invalid_params":
        if not isinstance(args.get("ticket_price"), (int, float)) or args["ticket_price"] <= 0:
            raise ValueError(f"参数 ticket_price 非法: {args.get('ticket_price')}")
        return {"fee": args["ticket_price"] * 0.05}
    ...
    if tool_name == "partial_failure":
        return {"trains": [{"no": "G107", "left": 5}],
                "warnings": ["G109 数据获取失败，仅展示部分结果"]}
    return {"ok": True}
...
test_scenarios = [
    ...
    ("invalid_params", {"ticket_price": -100}, "参数非法"),
    ("not_found", {}, "工具未注册"),
    ("rate_limited", {}, "限流"),
