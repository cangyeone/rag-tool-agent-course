...
def load_course_docs() -> list[dict]:
    ...
    business = [
        ...
        {
            "title": "变更规则",
            "content": "服务开始前可免费变更一次。变更后的订单日期在春运期间的不办理退款款。变更只能变更日期、订单编号和服务类型，不能变更出发站和到达站。",
        },
        {
            "title": "学生优惠票规则",
        ...
        {
            "title": "服务点信息",
            "content": "北京南、上海虹桥、杭州东等服务网点在系统中同时存在中文站名、拼音、服务点编码和城市信息。检索服务点资料时，关键词和语义检索可以结合使用。",
        },
        {
            "title": "客服质检标准",
        ...
        {
            "title": "混合检索",
            "content": "混合检索 = 关键词检索（BM25）+ 向量检索 + RRF 排名融合。关键词检索擅长精确匹配（如订单编号号 G107），向量检索擅长语义匹配（如'没票了怎么办'），两者互补。",
        },
        {
            "title": "模型后端选择",
