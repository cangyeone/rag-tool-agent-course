...
question = "帮我查一下今天的G107次服务流程还有票吗？如果不能买的话能不能候补申请？"
...
def query_ticket_availability(train_no, date="2025-06-19"):
    """模拟库存查询。"""
    return {
        "tool": "query_ticket_availability",
...
# 执行工具调用
tool_results = []
tool_results.append(query_ticket_availability("G107"))
tool_results.append(query_station_info("北京南"))
...
ticket_info = tool_results[0]["result"] if tool_results else None
station_info = tool_results[1]["result"] if len(tool_results) > 1 else None
...
G107 次服务流程 {ticket_info['from']} → {ticket_info['to']}，
目前商务座剩余 3 张，高级服务剩余 12 张，标准服务已售罄。
