"""09 输入图像的例子 —— 调用本地 Qwen3.5 图像理解模型。

学习目标：看懂图片如何作为 messages 输入给本地 Qwen3.5 多模态模型，
并完成一次真实的图片问答调用。

运行方式：
    python code/01_大模型基础/code/lesson_01_输入输出与对话格式/09_输入图像的例子.py
...
print("09 输入图像的例子 —— 调用本地 Qwen3.5 图像理解模型")
print("=" * 72)
...
print("\n二、准备演示图片")
image_dir = COURSE_ROOT / "code" / "01_大模型基础" / "code" / "lesson_01_输入输出与对话格式"
image_path = image_dir / "_demo_qwen35_vl_service_point_board.jpg"
...
draw.text((120, 185), "Train: ORD-1001", fill=(255, 225, 90), font=font_big)
...
question = "请读取图片中的信息，回答服务编号、起点、终点、办理窗口、服务窗口和时间。请用中文分点回答。"
