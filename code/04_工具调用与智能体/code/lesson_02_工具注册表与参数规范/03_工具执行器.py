...
def query_service_point(args):
    time.sleep(0.2)
    return {"code": "SP-A", "name": "服务点A"}
...
tools_map = {
    "query_order_statuss": query_order_statuss,
    "query_service_point": query_service_point,
    "calc_refund_fee": calc_refund_fee,
}

# ========== 执行策略1：串行执行 ==========
print("【策略1：串行执行 —— 适合有依赖关系】")
tool_calls_sequential = [
    {"id": "c1", "name": "query_service_point", "args": {"name": "服务点A"}},
    {"id": "c2", "name": "query_order_statuss", "args": {"from": "服务点A"}},
]
t0 = time.time()
results_seq = []
...
tool_calls_parallel = [
    {"id": "p1", "name": "query_service_point", "args": {"name": "服务点A"}},
    {"id": "p2", "name": "query_order_statuss", "args": {"from": "服务点A"}},
    {"id": "p3", "name": "calc_refund_fee", "args": {"price": 553}},
]
t0 = time.time()
results_par = []
threads = []
...
# 正常工具
print(f"  query_service_point: {json.dumps(execute_with_timeout('query_service_point', {}, timeout=1.0), ensure_ascii=False)}")
# 超时工具
