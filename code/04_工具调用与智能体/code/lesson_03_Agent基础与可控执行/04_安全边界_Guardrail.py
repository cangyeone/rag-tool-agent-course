...
BLOCKED_PATTERNS = [
    ...
    (r"(刷票|抢票|囤票|黄牛|脚本|爬虫)", "疑似恶意创建订单行为"),
    (r"(退钱|赔钱|投诉.*示例业务系统|起诉)", "敏感投诉内容（应转人工）"),
    (r"(炸弹|恐怖|攻击|破坏)", "安全威胁内容"),
]

ALLOWED_DOMAINS = ["创建订单", "查库存", "候补申请", "退款与变更", "服务点", "服务编号", "示例业务系统"]
...
class ToolGuard:
    ...
    def __init__(self):
        ...
        self.limits = {                 # 每次会话的最大调用次数
            "query_order_statuss": 20,
            "create_order": 3,          # 创建订单限制严格
            "submit_waitlist": 5,
            "cancel_order": 2,
        }
        self.dangerous_tools = {"create_order", "cancel_order", "refund_order"}
...
test_calls = [
    ("create_order", False),   # 未认证，应拒绝
    ("create_order", True),    # 已认证，应允许
    ("query_order_statuss", False),  # 查库存无需认证
]
...
OUTPUT_FORBIDDEN = [
    (r"\d{17}[\dXx]", "疑似身份证号泄露"),
    (r"1[3-9]\d{9}", "疑似手机号泄露"),
    (r"(保证|承诺|一定|肯定).*(能|会|可以).*(买到|有库存|候补申请成功)", "不得承诺创建订单结果"),
]
...
test_answers = [
    "ORD-1001 已无库存，候补申请不能保证成功，最终以 官方页面为准。",
    "保证能买到票！我们系统绝对可靠！",
