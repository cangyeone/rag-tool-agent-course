...
class QwenAgent:
    ...
    AGENT_PROMPT = (
        ...
        "1. 先判断需要调用哪个工具（query_order_statuss / calc_refund_fee / query_service_point / create_order）\n"
        "2. 提取工具所需参数\n"
        "3. 生成回答，回答中必须提及官方渠道，不能做虚假承诺。\n\n"
        "可用的工具：\n"
        "- query_order_statuss: 查询服务编号库存，参数 order_id, date\n"
        "- calc_refund_fee: 计算退款费用，参数 order_amount, hours_before\n"
        "- query_service_point: 查询服务点编码，参数 service_point_name\n"
        "- create_order: 提交创建订单订单，参数 order_id, seat\n\n"
...
# ========== 评测用例 ==========
eval_cases = [
    {"question": "ORD-1001 没票了还能怎么办？",
     "expected_tool": "query_order_statuss",
     "checks": ["answer_has_evidence", "answer_has_disclaimer", "no_false_promise"]},
    {"question": "退款要扣多少钱？",
     "expected_tool": "calc_refund_fee",
     "checks": ["answer_has_evidence", "no_false_promise"]},
    {"question": "服务点A的服务点编码是多少？",
     "expected_tool": "query_service_point",
     "checks": ["answer_has_evidence", "no_false_promise"]},
    {"question": "帮我订一张明天的 ORD-1001 标准服务",
