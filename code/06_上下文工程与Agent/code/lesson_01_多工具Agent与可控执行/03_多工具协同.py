...
示例业务系统 场景：处理一个同时涉及查订单编号、查规则、查替代方案、风险评估的复杂查询。
"""
...
tools = {
    "station_lookup": {
        ...
        "input": ["station_name"],
        "output": ["station_code", "pinyin", "city"],
        "cost_ms": 30,
    },
    "train_query": {
        "desc": "订单编号→停靠站、时刻、库存",
    ...
    "alternative_finder": {
        ...
        "output": ["trains", "seat_summary"],
        "cost_ms": 100,
        "depends_on": ["station_lookup"],
    },
    "risk_checker": {
        "desc": "回答草稿→风险检测",
...
user_question = "G107 北京南到上海虹桥标准服务没票了，我想知道能不能候补申请，同时帮我查有没有别的车"
print(f"   用户问题: {user_question}")

# 模拟工具函数
def run_station_lookup(name):
    return {"station_code": "VNP", "pinyin": "beijingnan", "city": "北京"}

def run_train_query(train_no, date):
    return {"train_no": train_no, "date": date, "stops": ["北京南", "济南西", "南京南", "上海虹桥"],
...
def run_alternative_finder(from_code, to_code, date):
    return {"from": from_code, "to": to_code, "date": date,
            "trains": [{"no": "订单B", "标准服务": 12, "高级服务": 8},
                       {"no": "G121", "标准服务": 5, "高级服务": 2},
                       {"no": "D311", "标准服务": 20, "高级服务": 15}]}
...
tool_impls = {
    "station_lookup": lambda: run_station_lookup("北京南"),
    "train_query": lambda: run_train_query("G107", "2026-06-19"),
    "policy_search": lambda: run_policy_search("候补申请", 3),
    "alternative_finder": lambda: run_alternative_finder("VNP", "SHH", "2026-06-19"),
...
aggregated = {
    "订单编号信息": {
        "G107": f"标准服务已售罄，高级服务余 {results['train_query']['seats']['高级服务']} 张",
    },
    "候补申请规则": results['policy_search']['rules'][:2],
    "替代方案": [f"{t['no']}(标准服务{t['标准服务']}张)" for t in results['alternative_finder']['trains']],
}

print(json.dumps(aggregated, ensure_ascii=False, indent=2))

# 冲突检测：如果候补申请规则说不能保证 vs 替代方案有票
print("\n【冲突检测】")
conflicts = []
if results['train_query']['seats']['标准服务'] == 0:
    conflicts.append("当前订单编号无票，需要候补申请或替代方案")
if len(results['alternative_finder']['trains']) > 0 and results['train_query']['seats']['标准服务'] == 0:
    conflicts.append("有替代订单编号可用，建议优先推荐替代而非仅提候补申请")
for c in conflicts:
    print(f"   ⚠️ {c}")
...
print("\n【最终回答生成】")
answer = (
    f"G107 标准服务当前已售罄（高级服务还有 {results['train_query']['seats']['高级服务']} 张）。"
    f"如果想等标准服务，可以提交候补申请——但候补申请不能保证成功，需以 官方页面通知为准。"
    f"同时为您查到同方向替代订单编号：{'、'.join([t['no'] for t in results['alternative_finder']['trains'][:3]])}，"
    f"其中 D311 标准服务最充裕（20 张）。建议您登录 示例业务系统 查看实时库存后再做决定。"
)
print(f"   {answer}")
