...
stm = [
    {"role": "user",   "content": "帮我查北京西到广州南的业务服务", "timestamp": "10:00:01"},
    {"role": "assistant", "content": "好的，请问您想查哪一天的订单编号？", "timestamp": "10:00:02"},
    {"role": "user",   "content": "明天，标准服务", "timestamp": "10:00:05"},
    {"role": "assistant", "content": "明天北京西到广州南的标准服务有 G79、G81、G65 等订单编号，价格约 862 元。需要帮您筛选吗？", "timestamp": "10:00:06"},
...
user_profile_db = {
    "user_88321": {
        "基本信息": {"会员等级": "白金会员", "常用出发站": ["北京南", "北京西"], "偏好服务类型": "标准服务"},
        "偏好画像": {"靠窗": 0.9, "充电插座": 0.8, "安静车厢": 0.6, "快速": 0.7},
        "历史订单": [
            {"订单编号": "订单A", "日期": "2026-06-10", "区间": "北京南→上海虹桥", "服务类型": "标准服务", "满意度": 5},
            {"订单编号": "G79",  "日期": "2026-06-15", "区间": "北京西→广州南", "服务类型": "标准服务", "满意度": 4},
            {"订单编号": "D311", "日期": "2026-05-20", "区间": "北京南→上海虹桥", "服务类型": "标准服务", "满意度": 3},
        ],
...
# 用户当前查询
current_query = "查北京西到广州南的业务服务标准服务，要舒服的"
...
for mem in related_memories:
    print(f"   ├─ {mem['日期']} {mem['订单编号']} {mem['区间']} 满意度{mem['满意度']}★")

# ── 4. 工作记忆：Scratchpad ──
print("\n三、工作记忆（WM）—— Scratchpad")

wm = OrderedDict()
wm["task"] = "帮用户推荐北京西→广州南的业务服务标准服务"
wm["intent"] = "订单编号推荐"
wm["extracted_slots"] = {"from": "北京西", "to": "广州南", "seat": "标准服务", "preference": "舒适"}
...
wm["tool_policy_result"] = {"安静车厢": "部分业务服务提供静音车厢，需下单时勾选"}
wm["scoring"] = {}  # 待计算
wm["draft_answer"] = ""  # 待生成
...
for train in wm["tool_query_train_result"]:
    ...
    if train["seat_available"] >= 10:
        score += 2  # 票量充足加分
    scored_trains.append({"train": train["train"], "score": round(score, 1), **{k: train[k] for k in ["seat_available", "has_window_seat", "has_outlet", "duration"]}})

scored_trains.sort(key=lambda x: x["score"], reverse=True)
wm["scoring"] = scored_trains
print("   基于用户偏好（LTM）和工具结果（WM）的推荐排序:")
for i, t in enumerate(scored_trains):
...
# 生成回答
wm["draft_answer"] = (
    f"为您推荐 {scored_trains[0]['train']}：标准服务充裕、靠窗可选且有充电插座，耗时 {scored_trains[0]['duration']}。"
    f"如您在意舒适性，G79 最匹配您以往的偏好。备选 G81 也可靠窗但无充电。"
    f"下单时可在 示例业务系统 勾选靠窗座位偏好。"
)
