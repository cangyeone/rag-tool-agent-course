...
route_tool_map = {
    "查库存": {
        "tools": ["service_point_lookup", "inventory_query"],
        "fallback_route": "客服转接",
    },
    "买票": {
        "tools": ["service_point_lookup", "inventory_query", "order_creator"],
        "fallback_route": "查库存",
    },
    "退款与变更": {
...
# ── 二、模拟工具实现 ──
def service_point_lookup(name):
    service_points = {"服务点A": "SP-A", "服务点B": "SP-B", "服务点C": "SP-C", "服务点D": "SP-D"}
    code = service_points.get(name)
    if code:
        return {"success": True, "service_point_code": code, "service_point_name": name}
    return {"success": False, "error": f"未找到服务点: {name}"}

def inventory_query(from_code, to_code, date):
    if random.random() < 0.15:  # 15%概率模拟失败
        return {"success": False, "error": "查询超时，请稍后重试"}
    return {
        "success": True,
        "items": [
            {"order_id": "ORD-1001", "seats_left": random.randint(0, 200), "price": 553},
            {"order_id": "ORD-1002", "seats_left": random.randint(0, 200), "price": 553},
        ],
    }

def refund_calculator(order_id, price, days_before):
    if days_before > 15:
        rate = 0.05
    elif days_before > 2:
        rate = 0.2
...
def order_lookup(order_id):
    ...
    return {"success": True, "order_id": order_id, "status": "已支付", "train": "ORD-1001"}

def service_point_info(service_point_name):
...
def order_creator(from_code, to_code, date, order_id=""):
...
def classify_route(question):
    ...
    if any(k in question for k in ["退", "变更", "手续费", "取消"]):
        return "退款与变更"
    if any(k in question for k in ["买", "创建订单", "订票", "创建订单"]):
        return "买票"
    if any(k in question for k in ["站", "寄存", "餐厅", "设施", "换乘", "地铁"]):
        return "站内服务"
    if any(k in question for k in ["密码", "账号", "登录", "认证"]):
        return "账号问题"
    if any(k in question for k in ["票", "服务编号", "库存", "时刻", "还有"]):
        return "查库存"
    return "客服转接"
    ...
    for s in ["服务点A", "服务点B", "服务点C", "服务点D"]:
        if s in question:
            if "from_location" not in params:
                params["from_location"] = s
            elif "to_location" not in params:
                params["to_location"] = s

    # 默认值
    params.setdefault("date", "2026-06-20")
    params.setdefault("order_amount", 553)
...
def prepare_tool_params(tool_name, params, previous_results):
    ...
    if tool_name == "service_point_lookup":
        name = params.get("from_location", "服务点A")
        tool_params["name"] = name
    elif tool_name == "inventory_query":
        from_result = previous_results.get("service_point_lookup", {})
        tool_params["from_code"] = from_result.get("service_point_code",
                                                    service_point_lookup(params.get("from_location", "服务点A")).get(
                                                        "service_point_code", "SP-A"))
        tool_params["to_code"] = "SP-B"  # 模拟
        tool_params["date"] = params.get("date", "2026-06-20")
    elif tool_name == "refund_calculator":
        tool_params["order_id"] = params.get("order_id", "ORD-1001")
        tool_params["price"] = params.get("order_amount", 553)
        tool_params["days_before"] = params.get("days_before", 5)
    elif tool_name == "order_lookup":
        tool_params["order_id"] = params.get("order_id", "未知")
    elif tool_name == "service_point_info":
        tool_params["service_point_name"] = params.get("from_location", "服务点A")
        ...
        tool_params["from_code"] = "SP-A"
        tool_params["to_code"] = "SP-B"
        tool_params["date"] = params.get("date", "2026-06-20")
        tool_params["order_id"] = params.get("order_id", "")
...
def generate_answer(route, results, success):
    """根据路由类型和工具结果生成回答。"""
    if route == "查库存":
        tq = results.get("inventory_query", {})
        if tq.get("success"):
            items = tq.get("items", [])
            train_str = "、".join(
                f"{t['order_id']}(余{t['seats_left']}张/¥{t['price']})" for t in items)
            return f"查询到以下服务编号: {train_str}。建议尽早创建订单。"
    ...
    if route == "退款与变更":
        ...
        if rc.get("success") and ol.get("success"):
            return (f"订单{ol['order_id']}({ol['train']})退款费用¥{rc['fee']}"
                    f"(费率{rc['rate']})，可退¥{rc['refund']}。")
        return "请提供正确的订单号以查询退款与变更详情。"

    if route == "买票":
        oc = results.get("order_creator", {})
        if oc.get("success"):
            return f"已为您创建订单 {oc['order_id']}，状态: {oc['status']}。请在15分钟内完成支付。"
        return "创建订单失败，请检查信息后重试。"

    if route == "站内服务":
        si = results.get("service_point_info", {})
        if si.get("success"):
...
# ── 五、批量测试 ──
test_questions = [
    "帮我查一下服务点A到上海的 ORD-1001 还有库存吗",
    "我要退 ORD-1001 的票，已经买了553块",
    "帮我买一张明天服务点C到服务点D的业务服务票",
]
...
print("2. 在 inventory_query 中故意让失败概率提高，观察重路由行为。")
