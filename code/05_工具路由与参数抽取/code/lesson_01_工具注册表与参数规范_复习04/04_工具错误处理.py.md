...
> 对应代码：`半天三节课_演示脚本/03_6月23日上午_工具注册表与多工具Agent/lesson_01_工具注册表与参数规范/04_工具错误处理.py`
...
# 第 3 步：按工具名执行。每个分支对应一种工具调用结果。
selected_tool = "order_advice"
if selected_tool == "service_point_code":
    result = {"service_point_name": service_point_name, "service_point_code": "SP-A"}
elif selected_tool == "order_advice":
    result = {"order_id": order_id, "advice": "查看同方向其他服务编号，或尝试候补申请，不能保证成功。"}
elif selected_tool == "refund_rule":
    result = {"rule": "退款与变更以订单页面规则为准。"}
else:
    result = {"error": "未知工具，请检查工具注册表。"}
print("四、工具执行结果")
