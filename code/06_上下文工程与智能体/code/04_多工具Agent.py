...
tools_registry = OrderedDict([
    ("search_service_point", {"desc": "根据服务点名称查编码和拼音", "params": ["service_point_name"]}),
    ("query_train", {"desc": "根据服务编号查停靠站、库存状态", "params": ["order_id", "date"]}),
    ("search_policy", {"desc": "查退款与变更、候补申请等业务规则", "params": ["keyword", "top_k"]}),
    ("check_alternatives", {"desc": "查同方向替代服务编号或中转方案", "params": ["from_location", "to_location", "date"]}),
...
# ── 第 1 步：意图识别 ──
user_question = "ORD-1001 服务点A到服务点B标准服务没票，能候补申请吗？还有没有别的车能走？"
print(f"\n【1. 意图识别】\n   用户输入: {user_question}")

def classify_intent(text):
    scores = {}
    if any(w in text for w in ["没票", "无票", "无库存", "满了"]):
        scores["库存查询"] = 0.9
...
if intent_result["primary"] == "库存查询":
    tool_plan.append({"id": "P1", "tool": "query_train", "params": {"order_id": "ORD-1001", "date": "2026-06-19"}, "depends_on": [], "why": "查当前服务编号库存"})
...
if intent_result.get("all", {}).get("替代服务编号", 0) > 0.5:
    tool_plan.append({"id": "P3", "tool": "check_alternatives", "params": {"from_location": "服务点A", "to_location": "服务点B", "date": "2026-06-19"}, "depends_on": [], "why": "帮用户查替代方案"})
...
def exec_tool(tool_name, params):
    ...
    if tool_name == "query_train":
        return {"order_id": params["order_id"], "date": params.get("date", ""), "seats": {"标准服务": 0, "高级服务": 3, "商务座": 2}, "status": "标准服务已无库存"}
        ...
        return {"from": params["from_location"], "to": params["to_location"], "date": params.get("date", ""), "alternatives": [{"order_id": "订单B", "seats": {"标准服务": 12}}, {"order_id": "ORD-1003", "seats": {"标准服务": 5}}, {"order_id": "ORD-2001", "seats": {"标准服务": 20}}]}
...
# 结构化整合
synthesis = {
    "服务编号状态": f"{train_info.get('order_id', 'ORD-1001')} 标准服务已无库存，高级服务余 {train_info.get('seats', {}).get('高级服务', '?')} 张",
    "候补申请规则": policy_info.get("hits", ["无"])[0] if policy_info.get("hits") else "无相关规则",
    "替代方案": [f"{a['order_id']}(标准服务余{a['seats'].get('标准服务', '?')}张)" for a in alt_info.get("alternatives", [])],
...
print(f"\n【5. 生成回答】")
final_answer = (
    f"ORD-1001 标准服务当前已无库存，高级服务还有少量库存。"
    f"如果坚持标准服务，可以尝试候补申请——候补申请不能保证成功，最终以 官方页面通知为准。"
    f"同时为您查到同方向替代服务编号：{'、'.join(synthesis['替代方案'])}，可登录 示例业务系统 查看实时库存。"
)
